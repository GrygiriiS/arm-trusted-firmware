...
static void scmi_pinctrl_report_attributes(struct scmi_msg *msg)
{
	...
	return_values.attrs_low = ((nr_groups << 16) & GENMASK(31, 16)) |
				  (nr_pins & GENMASK(15, 0));
	return_values.attrs_high = nr_functions & GENMASK(15, 0);
...
static void scmi_pinctrl_attributes(struct scmi_msg *msg)
{
	...
	case PIN_TYPE:
		...
		return_values.attributes = SCMI_PINCTRL_ATTR_NUM_ELEM(1);
	...
	case GROUP_TYPE:
		...
		return_values.attributes = SCMI_PINCTRL_ATTR_NUM_ELEM(nelems);
					    ...
					    struct scmi_pinctrl_assoc_p2a *rx)
{
	...
	for (counter = tx->index; counter < nr_pins; counter++, lcounter++) {
		...
		rx_size += sizeof(rx->array[0]);
		VERBOSE("group pins [%d] = %d\n", lcounter,
			rx->array[lcounter]);
	}

	rx->flags = SCMI_PINCTRL_ASSOC_REM(nr_pins - counter) |
		    SCMI_PINCTRL_ASSOC_RET(lcounter);
	rx->status = SCMI_SUCCESS;

	rx_size = round_up(rx_size, sizeof(uint32_t));
				 ...
				 struct scmi_pinctrl_assoc_p2a *rx)
{
	...
	rx->flags = SCMI_PINCTRL_ASSOC_REM(nr_groups - counter) |
		    SCMI_PINCTRL_ASSOC_RET(lcounter);
	rx->status = SCMI_SUCCESS;
...
static void scmi_pinctrl_config_get(struct scmi_msg *msg)
{
	...
	type = SCMI_PINCTRL_CFGG_SEL_TYPE(tx->attributes);
	cfg_flag = SCMI_PINCTRL_CFGG_FLAG(tx->attributes);
	scmi_conf_type = SCMI_PINCTRL_CFGG_TYPE(tx->attributes);
	...
	if (SCMI_PINCTRL_CFGG_SKIP(tx->attributes)) {
...
static void scmi_pinctrl_config_set(struct scmi_msg *msg)
{
	...
	type = SCMI_PINCTRL_CFG_SEL_TYPE(tx->attributes);
	nr_cfg = SCMI_PINCTRL_CFG_NUM(tx->attributes);
...
static void scmi_pinctrl_request(struct scmi_msg *msg)
{
	...
	type = SCMI_PINCTRL_REQ_SEL_TYPE(tx->flags);
...
static void scmi_pinctrl_release(struct scmi_msg *msg)
{
	...
	type = SCMI_PINCTRL_REL_SEL_TYPE(tx->flags);

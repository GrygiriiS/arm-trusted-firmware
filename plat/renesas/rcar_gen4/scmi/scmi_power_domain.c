...
static int rcar_gen4_sysc_power(uint32_t pdr, bool on)
{
	int ret = SCMI_SUCCESS;
	uint32_t val;

	INFO("start sysc power %s domain:%d status:%08x\n", on ? "on" : "off",
		 pdr, mmio_read_32(R8A779G0_SYSC_BASE + PDRSR(pdr)));

	if (on && r8a779g0_areas[pdr].parent >= 0) {
		ret = rcar_gen4_sysc_power(r8a779g0_areas[pdr].parent, on);
		if (ret) {
	...
	if (!on && r8a779g0_areas[pdr].parent >= 0) {
		ret = rcar_gen4_sysc_power(r8a779g0_areas[pdr].parent, on);
		if (ret) {
			ERROR("paernt_domain:%d off gen err (%d)\n", r8a779g0_areas[pdr].parent, ret);
			goto exit_unlock;
		}
	}

exit_unlock:
	INFO("sysc power %s domain:%d status:%08x (%d)\n", on ? "on" : "off",
	     pdr, mmio_read_32(R8A779G0_SYSC_BASE + PDRSR(pdr)), ret);
				    ...
				    unsigned int pd_id)
{
	...
	if (!pd->name) {
		/* reserved/unsupported no access - return ON*/
		return 0;
	}
			       ...
			       unsigned int state)
{
	...
	pd = &r8a779g0_areas[pd_id];
	if (!pd->name) {
		/* reserved/unsupported no access */
	...
	if (pd->flags & PD_ALWAYS_ON_ALL) {
		/* always on PD */
		return on ? SCMI_SUCCESS : SCMI_BUSY;
	}
...
//	}
	...
	ret = rcar_gen4_sysc_power(pd_id, on);
...
/*
 * TODO: Enable PD on init if PD_FORCE_ALWAYS_ON
 */
void rcar_init_scmi_power_domains(void)
{
	...
	INFO("sysc power ON status status0:%08x status1:%08x\n",
	     mmio_read_32(R8A779G0_SYSC_BASE + SYSCPONSR(0)),
	     mmio_read_32(R8A779G0_SYSC_BASE + SYSCPONSR(1)));
	INFO("sysc power OFF status status0:%08x status1:%08x\n",
...
//		if (pd->flags & PD_FORCE_ALWAYS_ON) {
			ret = rcar_gen4_sysc_power(i, true);
			if (ret) {
				ERROR("domain:%d gen err (%d)\n", i, ret);
			}
//		}

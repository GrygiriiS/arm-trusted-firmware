...
int32_t plat_scmi_clock_get_state(unsigned int agent_id, unsigned int scmi_id)
{
	...
	mod = &r8a779g0_mod_clks[GET_MOD_CLK_IDX(scmi_id)];
	...
	reg_ofs = R8A779G0_REG_ID(mod->id);
	bit_ofs = R8A779G0_REG_BIT_ID(mod->id);
	enabled = rcar_mssr_clock_state(reg_ofs, bit_ofs);
	if (mod->enabled != enabled) {
		VERBOSE("scmi: get state unsync scmi_id:%u mod:%u real::%d\n",
			scmi_id, mod->enabled, enabled);
		mod->enabled = enabled;
	}

	return (int32_t)mod->enabled;
}
